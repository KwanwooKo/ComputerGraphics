...
def display():
    ...
    R1 = Rotation.from_matrix(cow2wld1[0:3, 0:3])  # cow2wld1
    R2 = Rotation.from_matrix(cow2wld2[0:3, 0:3])  # cow2wld2

    delta = (R1.inv()*R2).as_rotvec() * animTime     # R1^(-1) * R2

    v = Rotation.from_rotvec(delta)
    R3 = R1 * v

    cowPosition1 = getTranslation(cow2wld1)
    cowPosition2 = getTranslation(cow2wld2)

    cowPosition = (1 - animTime) * cowPosition1 + (animTime) * cowPosition2
    # drawCow(matrix4(R3.as_matrix(), getTranslation(cow2wld2)))
    drawCow(matrix4(R3.as_matrix(), cowPosition))
...
def main():
    ...
    width = 800
    height = 600
    window = glfw.create_window(width, height, '2020060100-7-1', None, None)
    if not window:
        glfw.terminate()
        sys.exit(-1)
